...
class Bumps(composer.Arena):
  """A flat arena with bumps."""
  def _build(self, size=(10, 10), aesthetic='default', name='bowl'):
    ...
    if aesthetic != 'default':
      ...
      self._texture = self._mjcf_root.asset.add(
        'texture', name='aesthetic_texture', file=ground_info.file, type=ground_info.type
      )
      self._material = self._mjcf_root.asset.add(
        'material', name='aesthetic_material', texture=self._texture, texuniform='true'
      )
      ...
      self._ground_geom = self._mjcf_root.worldbody.add(
        'geom', type='plane', name='groundplane', size=list(size) + [0.5], material=self._material
      )
    else:
      self._skybox = self._mjcf_root.asset.add(
        'texture',
    ...
    self.mjcf_model.visual.map.set_attributes(znear=0.00025, zfar=50.)
