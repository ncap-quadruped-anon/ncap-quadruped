  ...
  """
    ...
    """
        ...
        if motor_control_mode is robot_config.MotorControlMode.PWM:
            ...
            pwm = motor_commands + np.minimum(pd_max, 0) + np.maximum(
                pd_min, 0)
        else:
            pwm = -1 * kp * (motor_angle -
                             motor_commands) - kd * motor_velocity
    ...
    """
        ...
        if self._torque_limits is not None:
            observed_torque = np.clip(observed_torque,
                                      -1.0 * self._torque_limits,
                                      self._torque_limits)
        ...
        voltage_net = np.clip(
            ...
            np.asarray(true_motor_velocity), -VOLTAGE_CLIPPING,
            VOLTAGE_CLIPPING)
