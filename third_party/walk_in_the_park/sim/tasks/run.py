...
from walk_in_the_park.sim.arenas import HField
from walk_in_the_park.sim.tasks.utils import _find_non_contacting_height

DEFAULT_CONTROL_TIMESTEP = 0.03
DEFAULT_PHYSICS_TIMESTEP = 0.001


def get_run_reward_bounded(x_velocity: float, move_speed: float, cos_pitch: float, dyaw: float):
...
def get_run_reward_unbounded(x_velocity: float, move_speed: float, cos_pitch: float, dyaw: float):
    x = cos_pitch * x_velocity
    reward = np.clip(0.5 * move_speed * (x + move_speed), 0, None)
    reward -= 0.1 * np.abs(dyaw)
    return reward
    # return 10 * reward  # [0, 1] => [0, 10]
...
class Run(composer.Task):
    ...
    def __init__(self,
                 ...
                 floor_friction: Tuple[float, float, float] = (1, 0.005, 0.0001),
        ...
        self._move_speed = target_vx
        self._reward_fn = get_run_reward_bounded if reward_bounded else get_run_reward_unbounded
    ...
    def get_reward(self, physics):
        ...
        return self._reward_fn(x_velocity=velocimeter.sensordata[0],
    ...
    def initialize_episode_mjcf(self, random_state):
        ...
        # Original: This sets all geoms (floor, robot, and all). The issue, likely unrecognized by
        # the authors, is that the robot's foot friction is too high, so this bug was fixing it.
        for geom in self._floor.mjcf_model.find_all('geom'):
            geom.friction = new_friction

        # Sufficient: Just changing the foot frictions is enough to fix the problems.
        # for leg in ('FR', 'FL', 'RR', 'RL'):
        #     geom = self._floor.mjcf_model.worldbody.find('body', f'a1/{leg}_calf').get_children('geom')[-1]
        #     # geom.friction = (0.6, 0.3, 0.3)
        #     geom.friction = new_friction
